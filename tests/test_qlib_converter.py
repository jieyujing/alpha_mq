...
class TestFFillNoLookahead:
    ...
    def test_ffill_uses_pub_date_not_rpt_date(self):
        ...
        # 验证：1/24（披露前）应为null
        val_0124 = result.filter(pl.col("date") == pl.date(2024, 1, 24))["total_assets"][0]
        assert val_0124 is None, f"1/24应在披露前，期望null，实际{val_0124}"

        # 验证：1/25（披露日）开始有值100
        val_0125 = result.filter(pl.col("date") == pl.date(2024, 1, 25))["total_assets"][0]
        assert val_0125 == 100.0, f"1/25披露日应有值100，实际{val_0125}"

        # 验证：4/19仍使用旧值100（不是150！）
        val_0419 = result.filter(pl.col("date") == pl.date(2024, 4, 19))["total_assets"][0]
        assert val_0419 == 100.0, f"4/19不应看到4/20披露的数据，期望100，实际{val_0419}"

        # 验证：4/20的数据更新为新值150
        val_0420 = result.filter(pl.col("date") == pl.date(2024, 4, 20))["total_assets"][0]
...
class TestQlibBinConverter:
    ...
    def test_preprocess_fundamentals(self):
        ...
        result = converter.preprocess_fundamentals(raw_fund, ["ttl_ast", "ttl_eqy"], "pub_date")

        assert result["symbol"].to_list() == ["SH600000"]
        assert "ttl_ast" in result.columns
        assert "ttl_eqy" in result.columns

    def test_preprocess_fundamentals_with_trade_date(self):
        """测试使用trade_date的每日数据预处理"""
        converter = QlibBinConverter(raw_dir="data/raw", output_dir="data/csv_source")
        ...
        result = converter.preprocess_fundamentals(raw_fund, ["pe_ttm", "pb_lyr"], "trade_date")
