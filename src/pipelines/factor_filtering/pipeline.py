...
from pipelines.factor_filtering.steps.step06_representative import (
...
class FactorFilteringPipeline(DataPipeline):
    ...
    def load_data(self):
        ...
        # 初始化共享的 FilteringContext
        self.ctx = FilteringContext(df=self.df)
    ...
    def run_data_qa(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.qa_report = self.ctx.qa_report
    ...
    def run_preprocess(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.preprocess_report = self.ctx.preprocess_report
    ...
    def run_profiling(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.ic_metrics = self.ctx.ic_metrics
        valid_ics = [
            abs(m["mean_rank_ic"])
            for m in self.ic_metrics.values()
            if m.get("mean_rank_ic")
        ]
        mean_ic = sum(valid_ics) / max(len(valid_ics), 1) if valid_ics else 0
        logging.info(
            f"Ring 2 profiling done. Mean |IC|={mean_ic:.4f} across {len(self.ic_metrics)} factors"
        )
    ...
    def run_filter(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.filter_report = self.ctx.filter_report
    ...
    def run_stability(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.stability_report = self.ctx.stability_report
        logging.info(
            f"Ring 4 stability done. Factors checked: {len(self.stability_report)}"
        )
    ...
    def run_clustering(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.cluster_report = self.ctx.cluster_report
    ...
    def run_selection(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.selection_report = self.ctx.reports["selection_report"]
        logging.info(
            f"Ring 6 selection done. Selected: {self.selection_report.get('selected_count', 0)} factors"
        )
    ...
    def run_portfolio(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.portfolio_report = self.ctx.reports["portfolio_report"]
    ...
    def run_ml_importance(self):
        ...
        self.ctx = step.process(self.ctx)
        self.df = self.ctx.df
        self.ml_report = self.ctx.reports["ml_report"]
        top3 = sorted(
            self.ml_report.get("importance", {}).items(),
            key=lambda x: x[1],
            reverse=True,
        )[:3]
    ...
    def generate_report(self):
        ...
        lines.append(
            f"**Data Source**: {self.config.get('data', {}).get('factor_path', 'N/A')}"
        )
        ...
        lines.append("")
        lines.append(
            f"- Transforms applied: {self.preprocess_report.get('transform_applied', [])}"
        )
        lines.append("")
        ...
        sorted_ic = sorted(
            ...
            key=lambda x: abs(x[1].get("mean_rank_ic", 0)),
            reverse=True,
        )
        ...
        for rank, (feat, s) in enumerate(stable[:10], 1):
            ...
            if yearly:
                ...
                ic_range = "N/A"
            lines.append(
                f"| {rank} | {feat} | {s.get('stability_score', 0):.4f} | {ic_range} |"
            )
        lines.append("")
        ...
        lines.append(
            f"- Selected representatives: {self.selection_report.get('selected_count', 0)}"
        )
        lines.append("")
        ...
        try:
            ...
            pdf.cell(
                200,
                7,
                text=f"- Rejected factors: {len(self.qa_report.get('rejected', []))}",
                ln=True,
            )
            pdf.ln(5)
        ...
        factor_cols = [
            ...
            if c not in ("datetime", "instrument") and not c.startswith("label")
        ]
        all_cols = (
            ["datetime", "instrument"]
            + factor_cols
            + [c for c in self.df.columns if c.startswith("label")]
        )
        ...
        logging.info(
            f"Filtered factor pool saved to {filtered_path} ({len(factor_cols)} factors)"
        )
        ...
        selection_log = {
            ...
            "stability": {
                k: {kk: vv for kk, vv in v.items() if kk != "yearly_ic"}
                for k, v in self.stability_report.items()
            },
            ...
            "ml_importance": {
                ...
                "permutation_importance": self.ml_report.get(
        ...
        log_path.write_text(
            json.dumps(selection_log, indent=2, cls=_NumpyEncoder, default=str),
            encoding="utf-8",
        )
