...
class StabilityChecker:
    ...
    def _compute_daily_ic_all(self, df: pl.DataFrame, factor_cols: list[str], label_col: str) -> pl.DataFrame:
        """向量化计算所有因子的每日 IC。"""
    ...
    def process(self, df: pl.DataFrame) -> tuple[pl.DataFrame, dict]:
        ...
        max_rank_df = df_strat.group_by("datetime").agg(pl.col("size_rank").max().alias("max_rank"))
        df_strat = df_strat.join(max_rank_df, on="datetime").with_columns(
            pl.when(pl.col("size_rank") <= pl.col("max_rank") * 0.33).then(pl.lit("small"))
            .when(pl.col("size_rank") <= pl.col("max_rank") * 0.66).then(pl.lit("mid"))
        ...
        for col in factor_cols:
            ...
            ic_series = daily_ic_df.select(pl.col(col)).to_series().drop_nulls().drop_nans()
            if len(ic_series) < 2:
                stability[col] = {"stability_score": 0.0}
                continue

            # 年度 IC
            yearly_ic_df = (daily_ic_df
                .select([pl.col("datetime").dt.year().alias("year"), pl.col(col)])
                .filter(pl.col(col).is_not_nan())
                .group_by("year")
                .agg(pl.col(col).mean().alias("ic")))
            yearly_ic = {int(row["year"]): float(row["ic"] or 0) for row in yearly_ic_df.iter_rows(named=True)}
            ...
            # 市值分层 IC
            size_strat = (df_strat
                .filter(pl.col(col).is_not_nan() & pl.col(label_col).is_not_nan())
                .group_by("size_group")
                .agg(pl.corr(pl.col(col), pl.col(label_col), method="spearman").alias("ic")))
            size_ic = {row["size_group"]: float(row["ic"] or 0) for row in size_strat.iter_rows(named=True)}

            # 稳定性得分
            ic_values = [v for v in yearly_ic.values() if not np.isnan(v)]
            year_consistency = 1.0 - (float(pl.Series(ic_values).std()) if len(ic_values) > 1 else 0.0)
            ...
            size_consistency = 1.0 - (float(pl.Series(size_values).std()) if len(size_values) > 1 else 0.0)
